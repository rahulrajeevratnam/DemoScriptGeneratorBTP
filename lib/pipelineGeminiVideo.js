...
async function run({ jobId, videoPath, videoUploadKey, description, template, generateHtmlOutput, jobs }) {
  ...
  const screenshotMap = await buildTimestampScreenshots({
    scriptData, videoPath, screenshotsDir, log, durationSeconds: quality.duration
  });
...
// FFmpeg's single-frame seek grab is unreliable in the last fraction of a
// second of a file, and the AI-identified timestamp can occasionally exceed
// the video's real duration entirely (seen in practice: Gemini returning
// timestamps well past the actual end). Leave a safety margin off the true
// end and retry once at an earlier point if the grab still fails.
const END_OF_VIDEO_SAFETY_MARGIN_S = 0.5;

async function buildTimestampScreenshots({ scriptData, videoPath, screenshotsDir, log, durationSeconds }) {
  const screenshotMap = {};
  let stepGlobal = 0;
  const maxSafeTs = durationSeconds > 0 ? Math.max(0, durationSeconds - END_OF_VIDEO_SAFETY_MARGIN_S) : null;
  ...
  for (const section of scriptData) {
    for (const subSection of section.subSections) {
      for (const action of subSection.actions) {
        ...
        const rawTs = typeof action.timestamp === 'number' ? action.timestamp : 0;
        let ts = rawTs;
        if (maxSafeTs !== null && ts > maxSafeTs) {
          log(`   ⚠️  Step ${stepGlobal} timestamp ${ts.toFixed(1)}s exceeds video duration (${durationSeconds.toFixed(1)}s) — clamping to ${maxSafeTs.toFixed(1)}s`, 'warn');
          ts = maxSafeTs;
        }
        const destPath = path.join(screenshotsDir, `${stepKey}.jpg`);

        try {
          await grabFrameWithRetry(videoPath, ts, destPath);
...
async function grabFrameWithRetry(videoPath, ts, destPath) {
  try {
    await extractFrameAtTimestamp(videoPath, ts, destPath);
  } catch (err) {
    const retryTs = Math.max(0, ts - 1);
    if (retryTs === ts) throw err;
    await extractFrameAtTimestamp(videoPath, retryTs, destPath);
  }
}
