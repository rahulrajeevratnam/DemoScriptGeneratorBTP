...
// Model deployed in the SAP AI Core Generative AI Hub (Claude via Anthropic on AI Core).
// Verify the exact name against your AI Core model catalog / orchestration deployment
// and override via env var if it differs (e.g. 'anthropic--claude-4-sonnet').
const MODEL_NAME = process.env.AICORE_ORCHESTRATION_MODEL || 'anthropic--claude-4.6-sonnet';
const MAX_TOKENS = parseInt(process.env.AICORE_MAX_TOKENS, 10) || 8192;

// @sap-ai-sdk/orchestration is ESM-only — load it lazily via dynamic import()
// so the rest of this CommonJS codebase doesn't need to change.
...
// Errors from @sap-ai-sdk/orchestration wrap the underlying HTTP client
// error (axios) as `.cause`, and the actual AI Core error body — the only
// place that says *why* a request was rejected — lives at
// `err.cause.response.data`. err.message alone is just "Request failed
// with status code 400" and tells us nothing.
  ...
  for (let batchIdx = 0; batchIdx < batches.length; batchIdx++) {
    ...
    for (const framePath of batch) {
      try {
        ...
        imageContent.push({ type: 'image_url', image_url: { url: `data:${mediaType};base64,${base64}` } });
    ...
    try {
      ...
      console.error(`[aiAnalyser] Batch ${batchIdx + 1} full error:`, JSON.stringify(detail, null, 2));
      log(`   ⚠️  AI batch ${batchIdx + 1} error: ${err.message} — ${detail.summary}`, 'warn');
    }
  }
