...
async function run({ jobId, videoPath, videoUploadKey, description, template, generateHtmlOutput, jobs }) {
  ...
  // screenshotFrames stay at native/source resolution — they become the
  // final screenshots. aiFrames are a separate, smaller copy sent to
  // Claude only (keeps AI Core cost/latency down; does not affect
  // screenshot quality).
  log('📽️  Extracting frames from video...');
  const { screenshotFrames, aiFrames } = await extractFrames(videoPath, framesDir, log);
  log(`✅ Extracted ${screenshotFrames.length} frame(s)`);
  ...
  const scriptData = await analyseVideo(aiFrames, description, log, templateText);
  ...
  const screenshotMap = await buildFrameScreenshots({ scriptData, frames: screenshotFrames, screenshotsDir, log });
