  ...
  return new Promise((resolve, reject) => {
    ffmpeg(videoPath)
      ...
      .outputOptions([
        ...
        '-vframes', '1',
        '-q:v', '2'
        // Deliberately no scale filter — this is a single seek + one-frame
        // grab, not a full-video decode pass, so it's cheap regardless of
        // source resolution. Kept at native resolution because this frame
        // becomes the final screenshot (see extractFrames' screenshotFrames
        // vs aiFrames split below) and clarity here is a hard requirement.
      ])
      .output(outputPath)
      .on('end', resolve)
...
async function extractFrames(videoPath, framesDir, log) {
  ...
  // Build a separate, smaller copy for the AI payload only. Claude doesn't
  // need native/4K resolution to read a UI, and sending smaller images
  // keeps AI Core token cost/latency down — that's a cost optimization,
  // not a quality one, so it must NOT be the same array used for the final
  // screenshots (screenshotFrames stays at native/dedupe resolution).
  const aiFrames = await buildAiCopies(dedupedFrames);

  return { screenshotFrames: dedupedFrames, aiFrames };
}

async function buildAiCopies(frames) {
  const resized = [];
  ...
  return new Promise((resolve, reject) => {
    ffmpeg(videoPath)
      ...
      .on('end', async () => {
        ...
        // Note: the fallback path (full-video decode for 1fps sampling)
        // downscales to 1080p at capture time for memory safety, unlike the
        // primary path's native-resolution single-frame grabs — an accepted
        // trade-off since this only runs when scene detection fails entirely.
        const aiFrames = await buildAiCopies(rawFrames);
        resolve({ screenshotFrames: rawFrames, aiFrames });
      })
...
// Exposed for lib/pipelineGeminiVideo.js — grabs one native-resolution frame
// at a given timestamp, reused as-is from the primary pipeline. Attached as
// a property (not a shape change) so existing `require('./frameExtractor')`
// callers keep working unchanged.
module.exports.extractFrameAtTimestamp = extractFrameAtTimestamp;
