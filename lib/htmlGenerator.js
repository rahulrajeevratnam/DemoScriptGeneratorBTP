...
// 1920 balances screenshot clarity against HTML file size (every screenshot
// is inlined as base64) — raise further if source videos are high-res and
// file size isn't a concern, since annotatedMap images now carry native/
// source resolution through from frameExtractor's screenshotFrames.
const HTML_SCREENSHOT_MAX_WIDTH = 1920;

async function imageToBase64(imagePath) {
  if (!imagePath || !fs.existsSync(imagePath)) return null;
  try {
    const buf = await sharp(imagePath)
      .resize({ width: HTML_SCREENSHOT_MAX_WIDTH, withoutEnlargement: true })
      .jpeg({ quality: 90 })
      .toBuffer();
...
async function generateHtml({ scriptData, annotatedMap, description, log }) {
...
<div class="content">
  ...
  log('   HTML document built in memory');
  return html;
}
